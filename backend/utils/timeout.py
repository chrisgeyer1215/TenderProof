"""
Utilitários de timeout para operações longas.

Fornece decorators e context managers para limitar tempo de execução.
"""
import asyncio
import signal
import threading
...
class TimeoutError(Exception):
    """Exceção levantada quando uma operação excede o timeout."""
...
class OCRTimeoutError(TimeoutError):
    """Exceção específica para timeout de operações OCR."""
    pass


def with_timeout_sync(timeout_seconds: int, operation_name: str = "Operation"):
    """
    Decorator para adicionar timeout a funções síncronas.

    Usa threading para implementar timeout em funções bloqueantes.
...
def timeout_context(timeout_seconds: int, operation_name: str = "Operation"):
    """
    Context manager para timeout em bloco de código.

    Funciona apenas em sistemas Unix (usa SIGALRM).
    Em Windows, o timeout não será aplicado mas não levantará erro.

    Args:
        timeout_seconds: Tempo máximo de execução
        operation_name: Nome da operação para logging
    ...
    if hasattr(signal, 'SIGALRM'):
        ...
        # Windows: timeout via signal não disponível
        # Usar with_timeout_sync() ou run_with_timeout() para timeout cross-platform
        import os
        env = os.environ.get("ENVIRONMENT", "development")
        if env == "production":
            logger.warning(
                f"[TIMEOUT] SIGALRM não disponível em Windows para {operation_name}. "
                "Considere usar with_timeout_sync() para timeout cross-platform."
            )
        else:
            logger.debug(f"Timeout via signal não disponível neste sistema para {operation_name}")
        yield
...
def run_with_timeout(
    ...
    **kwargs
    ...
    """
    Executa uma função com timeout usando threading.

    Função utilitária para aplicar timeout sem usar decorator.
