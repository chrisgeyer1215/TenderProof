"""
Processador de atestados de capacidade tecnica.
Extrai servicos, quantidades e dados de documentos de atestado.
"""
...
logger = get_logger('services.atestado_processor')
...
class AtestadoProcessor:
    ...
    Extrai servicos usando abordagem hibrida:
    ...
    def __init__(self, document_processor=None):
        ...
        Args:
            document_processor: Referencia ao DocumentProcessor para helpers
        """
        self._doc_processor = document_processor

    def set_document_processor(self, document_processor):
        """Define referencia ao DocumentProcessor."""
        self._doc_processor = document_processor
    ...
    def process(
        ...
        Args:
            ...
            use_vision: Se True, usa Vision AI quando disponivel
        ...
        # 2. Extrair servicos de tabelas (cascata)
    ...
    def _clear_item_code_quantities(self, servicos: list) -> int:
        """Limpa quantidades que parecem ser codigos de item."""
    ...
    def _enrich_servicos_from_text(
        ...
        """Enriquece servicos com dados extraidos do texto."""
        if not self._doc_processor:
            return servicos_raw

        dp = self._doc_processor

        # Determinar se text_section esta habilitado
        ...
        # Processar texto se nao for escaneado e tabela foi usada
        if (
            isinstance(doc_analysis, dict)
            and not doc_analysis.get("is_scanned")
            and table_used
        ):
            # Dividir texto por paginas (chamar text_extraction_service diretamente)
...
# Singleton (sera configurado depois com document_processor)
